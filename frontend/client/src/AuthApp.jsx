    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', background: 'var(--bg-1)' }}>
      ...
      <div style={{ display: 'flex', flex: 1, height: 'calc(100vh - 56px)', overflow: 'hidden' }}>
        ...
        <aside style={{
          ...
          {/* BRIEF */}
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '8px' }}>Brief</div>
          <textarea
            value={prompt}
            ...
            onFocus={(e) => { e.target.style.borderColor = 'rgba(255,45,120,0.4)'; e.target.style.boxShadow = '0 0 0 3px rgba(255,45,120,0.08)'; }}
          ...
          <button
            ...
            style={{
              ...
              color: enhancing ? 'var(--text-3)' : 'var(--text-2)',
              fontFamily: 'Inter, sans-serif',
              fontSize: '12px',
              fontWeight: 500,
            ...
            onMouseEnter={(e) => { if (prompt.trim() && !enhancing) { e.currentTarget.style.borderColor = 'var(--border-pink)'; e.currentTarget.style.color = 'var(--pink)'; }}}
            onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border-1)'; e.currentTarget.style.color = enhancing ? 'var(--text-3)' : 'var(--text-2)'; }}
          >
          ...
          {/* MOOD */}
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '8px', marginTop: '20px' }}>Mood</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '6px' }}>
            {MOODS.map((m) => (
              <button
                ...
                style={{
                  borderRadius: '8px',
                  border: mood === m.value ? '1.5px solid var(--border-pink)' : '1.5px solid var(--border-1)',
                  fontFamily: 'Inter, sans-serif',
                  fontSize: '13px',
                  fontWeight: 500,
                  padding: '7px 10px',
                  color: mood === m.value ? 'var(--pink)' : 'var(--text-2)',
                  background: mood === m.value ? 'var(--pink-soft)' : 'transparent',
                  cursor: 'pointer',
                  transition: 'all 0.15s',
                }}
              >{m.label}</button>
          ...
          {/* FORMAT */}
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '8px', marginTop: '20px' }}>Format</div>
            ...
            {[['portrait', '9:16 Portrait'], ['landscape', '16:9 Landscape']].map(([opt, label]) => (
              <button
                ...
                style={{
                  flex: 1,
                  borderRadius: '8px',
                  border: orientation === opt ? '1.5px solid var(--pink)' : '1.5px solid var(--border-1)',
                  fontFamily: 'Inter, sans-serif',
                  fontSize: '13px',
                  fontWeight: 500,
                  padding: '7px 10px',
                  color: orientation === opt ? '#fff' : 'var(--text-2)',
                  background: orientation === opt ? 'var(--pink)' : 'transparent',
                  cursor: 'pointer',
                  transition: 'all 0.15s',
                }}
              >{label}</button>
          ...
          {/* TRANSITIONS */}
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '8px', marginTop: '20px' }}>Transitions</div>
          <select
          ...
          <div style={{ marginTop: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)', letterSpacing: '0.15em', textTransform: 'uppercase' }}>Narration</div>
            <button
              type="button"
              ...
              style={{
                ...
                borderRadius: '11px',
                border: 'none',
                background: aiNarration ? 'var(--pink)' : 'var(--border-1)',
                cursor: 'pointer',
                position: 'relative',
          ...
          {aiNarration && (
            <>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)', letterSpacing: '0.15em', textTransform: 'uppercase', marginTop: '20px', marginBottom: '8px' }}>Voice</div>
                ...
                {[['ai', 'AI Voice'], ['upload', 'Upload'], ['record', 'Record']].map(([val, label]) => (
                  <button
                    ...
                    style={{
                      flex: 1,
                      borderRadius: '8px',
                      border: voiceMode === val ? '1.5px solid var(--pink)' : '1.5px solid var(--border-1)',
                      fontFamily: 'Inter, sans-serif',
                      fontSize: '11px',
                      fontWeight: 500,
                      padding: '6px 4px',
                      color: voiceMode === val ? '#fff' : 'var(--text-2)',
                      background: voiceMode === val ? 'var(--pink)' : 'transparent',
                      cursor: 'pointer',
                      transition: 'all 0.15s',
                    }}
                  >{label}</button>
              ...
              {voiceMode === 'upload' && (
                <div>
                  <label style={{
                    ...
                    color: customVoiceName ? 'var(--text-1)' : 'var(--text-3)',
                    fontFamily: 'Inter, sans-serif',
                    fontSize: '12px',
                    cursor: 'pointer',
                    textAlign: 'center',
                  ...
                  {customVoiceName && (
                    <button
                      ...
                      style={{ marginTop: '4px', background: 'none', border: 'none', color: 'var(--text-3)', fontSize: '11px', cursor: 'pointer' }}
                    >✕ Remove</button>
                  )}
                </div>
              )}
                ...
                <div>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      padding: '8px 12px',
                      borderRadius: '8px',
                      border: `1.5px solid ${recording ? 'var(--border-pink)' : 'var(--border-1)'}`,
                      background: recording ? 'var(--pink-soft)' : 'transparent',
                      color: recording ? 'var(--pink)' : 'var(--text-2)',
                      fontFamily: 'Inter, sans-serif',
                      fontSize: '12px',
                      fontWeight: 500,
                      cursor: 'pointer',
                  ...
                  >
                    {recording ? (
                      <>
                        <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'var(--pink)', animation: 'blink 1s step-end infinite', flexShrink: 0 }} />
                  ...
                  {customVoiceName && !recording && (
                    <div style={{ marginTop: '4px', fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)' }}>✓ Recording ready</div>
                  )}
                </div>
              )}
            </>
            ...
            {[['ai', 'AI Pick'], ['upload', 'Upload']].map(([val, label]) => (
              <button
                ...
                style={{
                  flex: 1,
                  borderRadius: '8px',
                  border: musicMode === val ? '1.5px solid var(--pink)' : '1.5px solid var(--border-1)',
                  fontFamily: 'Inter, sans-serif',
                  fontSize: '12px',
                  fontWeight: 500,
                  padding: '7px 10px',
                  color: musicMode === val ? '#fff' : 'var(--text-2)',
                  background: musicMode === val ? 'var(--pink)' : 'transparent',
                  cursor: 'pointer',
                  transition: 'all 0.15s',
                }}
              >{label}</button>
          ...
          {musicMode === 'upload' && (
            <div>
              <label style={{
                ...
                borderRadius: '8px',
                border: '1.5px dashed var(--border-1)',
                color: customMusicName ? 'var(--text-1)' : 'var(--text-3)',
                fontFamily: 'Inter, sans-serif',
                fontSize: '12px',
                cursor: 'pointer',
                textAlign: 'center',
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '20px', marginBottom: '8px' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)', letterSpacing: '0.15em', textTransform: 'uppercase' }}>Brand Kit</div>
            <a href="/brand-kit" style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--pink)', letterSpacing: '0.08em' }}>Edit →</a>
          </div>

          {brandKit?.logo_url ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', background: 'var(--bg-3)', borderRadius: '8px', border: '1px solid var(--border-1)', marginBottom: '8px' }}>
              <img
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: '11px', color: 'var(--text-1)' }}>Logo ready</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)', marginTop: '1px' }}>watermarks your reels</div>
              </div>
              <button
                type="button"
                onClick={() => setApplyBrandKit(v => !v)}
                aria-label="Toggle brand kit"
                style={{ width: '36px', height: '20px', borderRadius: '10px', border: 'none', background: applyBrandKit ? 'var(--pink)' : 'var(--border-1)', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}
              >
            ...
            <a href="/brand-kit" style={{ display: 'block', padding: '8px 10px', textAlign: 'center', borderRadius: '8px', border: '1.5px dashed var(--border-1)', color: 'var(--text-3)', fontFamily: 'Inter, sans-serif', fontSize: '11px', marginBottom: '8px', textDecoration: 'none', transition: 'border-color 0.15s' }}
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--border-pink)'; e.currentTarget.style.color = 'var(--pink)'; }}
          ...
          {applyBrandKit && brandKit?.logo_url && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '10px', background: 'var(--bg-2)', borderRadius: '8px', border: '1px solid var(--border-1)' }}>

              {/* Position grid */}
              <div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '8px', color: 'var(--text-4)', letterSpacing: '0.12em', marginBottom: '5px' }}>POSITION</div>
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        height: '24px',
                        borderRadius: '5px',
                        border: logoPosition === val ? '1.5px solid var(--pink)' : '1px solid var(--border-1)',
                        background: logoPosition === val ? 'var(--pink-soft)' : 'transparent',
                        color: logoPosition === val ? 'var(--pink)' : 'var(--text-3)',
                        fontSize: '12px',
                        cursor: 'pointer',
              ...
              {/* Size */}
              <div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '8px', color: 'var(--text-4)', letterSpacing: '0.12em', marginBottom: '5px' }}>SIZE</div>
                  ...
                  {[['S', 'Small'], ['M', 'Medium'], ['L', 'Large']].map(([val, label]) => (
                    <button
                      ...
                      style={{
                        flex: 1, padding: '4px',
                        borderRadius: '5px',
                        border: logoSize === val ? '1.5px solid var(--pink)' : '1px solid var(--border-1)',
                        background: logoSize === val ? 'var(--pink-soft)' : 'transparent',
                        color: logoSize === val ? 'var(--pink)' : 'var(--text-3)',
              ...
              {/* Timing */}
              <div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '8px', color: 'var(--text-4)', letterSpacing: '0.12em', marginBottom: '5px' }}>TIMING</div>
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: '5px',
                        border: logoTiming === val ? '1.5px solid var(--pink)' : '1px solid var(--border-1)',
                        background: logoTiming === val ? 'var(--pink-soft)' : 'transparent',
                        cursor: 'pointer', transition: 'all 0.12s',
                      }}
                    >
                      <div style={{ fontSize: '11px', fontWeight: 600, color: logoTiming === val ? 'var(--pink)' : 'var(--text-2)', fontFamily: 'Inter, sans-serif' }}>{label}</div>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: '8px', color: 'var(--text-4)', marginTop: '1px' }}>{hint}</div>
                    </button>
                  ))}
                </div>
              </div>
          ...
          {isOutOfCredits && (
            <div style={{ marginTop: '16px', padding: '10px 12px', borderRadius: '8px', border: '1px solid var(--border-pink)', background: 'var(--pink-soft)', fontSize: '12px', color: 'var(--text-2)' }}>
              No credits remaining.{' '}
              <a href="/pricing" style={{ color: 'var(--pink)', textDecoration: 'underline' }}>Upgrade →</a>
            </div>
          )}
          ...
          <button
            ...
            style={{
              ...
              boxShadow: loading || isOutOfCredits ? 'none' : 'var(--glow-pink)',
          ...
          <div style={{ marginTop: '10px', display: 'flex', alignItems: 'center', gap: '6px', fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)' }}>
            <span style={{
              ...
              background: loading ? 'var(--pink)' : '#22dd88',
          ...
          {error && (
            <div style={{ marginTop: '12px', padding: '10px 12px', borderRadius: '8px', border: '1px solid var(--border-pink)', background: 'var(--pink-soft)', fontSize: '12px', color: 'var(--pink)' }}>
              {error}
            </div>
          )}
        </aside>
          ...
          {!isGenerating && !isDone && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
              <div style={{ width: '280px', height: '400px', position: 'relative' }}>
                <div style={{ position: 'absolute', top: 0, left: 0, width: '20px', height: '20px', borderTop: '2px solid rgba(255,45,120,0.2)', borderLeft: '2px solid rgba(255,45,120,0.2)' }} />
                <div style={{ position: 'absolute', top: 0, right: 0, width: '20px', height: '20px', borderTop: '2px solid rgba(255,45,120,0.2)', borderRight: '2px solid rgba(255,45,120,0.2)' }} />
                <div style={{ position: 'absolute', bottom: 0, left: 0, width: '20px', height: '20px', borderBottom: '2px solid rgba(255,45,120,0.2)', borderLeft: '2px solid rgba(255,45,120,0.2)' }} />
                <div style={{ position: 'absolute', bottom: 0, right: 0, width: '20px', height: '20px', borderBottom: '2px solid rgba(255,45,120,0.2)', borderRight: '2px solid rgba(255,45,120,0.2)' }} />
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '12px' }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--text-3)', letterSpacing: '0.1em' }}>// AWAITING BRIEF</div>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text-4)' }}>Enter a prompt and hit generate</div>
                  <span style={{ color: 'var(--pink)', fontFamily: 'var(--font-mono)', animation: 'blink 1s step-end infinite' }}>▌</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div style={{ overflowY: 'auto', height: '100%', width: '100%' }}>
              <div style={{ maxWidth: '480px', margin: '0 auto', padding: '40px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                ...
                {showDownloadDialog && (
                  ...
                  >
                    <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border-2)', borderRadius: '16px', padding: '28px', width: '340px', maxWidth: '90vw', boxShadow: '0 24px 80px rgba(0,0,0,0.6)' }}>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--text-3)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '18px' }}>Save Reel</div>

                      {/* Title */}
                      <label style={{ display: 'block', fontFamily: 'Inter, sans-serif', fontSize: '12px', color: 'var(--text-2)', marginBottom: '6px' }}>Video title</label>
                      <input
                        type="text"
                        value={downloadTitle}
                        onChange={(e) => setDownloadTitle(e.target.value)}
                        placeholder="my-reel"
                        style={{ width: '100%', boxSizing: 'border-box', padding: '9px 12px', borderRadius: '8px', border: '1.5px solid var(--border-1)', background: 'var(--bg-3)', color: 'var(--text-1)', fontFamily: 'Inter, sans-serif', fontSize: '13px', outline: 'none', marginBottom: '16px' }}
                      ...
                      {typeof window !== 'undefined' && 'showSaveFilePicker' in window && (
                        <button
                          ...
                          style={{ width: '100%', padding: '10px', borderRadius: '10px', border: '1.5px solid var(--border-pink)', background: 'var(--pink-soft)', color: 'var(--pink)', fontFamily: 'Inter, sans-serif', fontSize: '13px', fontWeight: 600, cursor: 'pointer', marginBottom: '8px', transition: 'background 0.15s' }}
                          onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(255,45,120,0.18)'; }}
                          onMouseLeave={(e) => { e.currentTarget.style.background = 'var(--pink-soft)'; }}
                      ...
                      <a
                        ...
                        style={{ display: 'block', width: '100%', boxSizing: 'border-box', padding: '10px', borderRadius: '10px', border: '1.5px solid var(--border-2)', background: 'var(--bg-3)', color: 'var(--text-1)', fontFamily: 'Inter, sans-serif', fontSize: '13px', fontWeight: 500, textAlign: 'center', textDecoration: 'none', marginBottom: '8px', transition: 'border-color 0.15s' }}
                        onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--border-pink)'; }}
                      ...
                      <button
                        type="button"
                        onClick={() => setShowDownloadDialog(false)}
                        style={{ width: '100%', padding: '8px', borderRadius: '8px', border: 'none', background: 'transparent', color: 'var(--text-3)', fontFamily: 'Inter, sans-serif', fontSize: '12px', cursor: 'pointer' }}
                      >Cancel</button>
                    </div>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
                  <button
                    ...
                    style={{ flex: 1, textAlign: 'center', padding: '10px', borderRadius: '8px', background: 'var(--bg-3)', border: '1.5px solid var(--border-2)', color: 'var(--text-1)', fontFamily: 'Inter, sans-serif', fontSize: '13px', fontWeight: 500, cursor: 'pointer', transition: 'border-color 0.15s, color 0.15s' }}
                    onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--border-pink)'; e.currentTarget.style.color = 'var(--pink)'; }}
                  ...
                  <button
                    ...
                    style={{ flex: 1, padding: '10px', borderRadius: '8px', background: 'var(--bg-3)', border: '1.5px solid var(--border-2)', color: 'var(--text-1)', fontFamily: 'Inter, sans-serif', fontSize: '13px', fontWeight: 500, cursor: 'pointer', transition: 'border-color 0.15s, color 0.15s' }}
                    onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--border-pink)'; e.currentTarget.style.color = 'var(--pink)'; }}
                    onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border-2)'; e.currentTarget.style.color = 'var(--text-1)'; }}
                  >↗ Share</button>
                  <button
                ...
                {/* Satisfaction card */}
                <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border-1)', borderRadius: '12px', padding: '20px', marginTop: '16px' }}>
                  <div style={{ fontFamily: 'Inter, sans-serif', fontSize: '14px', fontWeight: 600, color: 'var(--text-1)', marginBottom: '14px' }}>
                    How did it turn out?
                  </div>
                  ...
                  {!satisfaction && (
                    <div style={{ display: 'flex', gap: '10px' }}>
                      ...
                      <button
                        ...
                        style={{ padding: '10px 20px', borderRadius: '8px', background: 'rgba(255,45,120,0.08)', border: '1.5px solid rgba(255,45,120,0.25)', color: 'var(--pink)', fontFamily: 'Inter, sans-serif', fontSize: '14px', fontWeight: 500, cursor: 'pointer' }}
                      >🔧 Needs work</button>
                    </div>
                  )}
                  ...
                  {satisfaction === 'love' && (
                    <div>
                      <p style={{ fontSize: '13px', color: 'var(--text-2)', margin: '0 0 12px' }}>Try a variation:</p>
                      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                        {['longer version', 'night time', 'slow-motion'].map((variant, i) => (
                          <button
                            ...
                            style={{ padding: '6px 14px', borderRadius: '6px', background: 'var(--pink-soft)', border: '1.5px solid var(--border-pink)', color: 'var(--pink)', fontFamily: 'Inter, sans-serif', fontSize: '12px', cursor: 'pointer' }}
                          >{variant}</button>
                        ))}
                      </div>
                    </div>
                ...
                <div style={{ display: 'flex', gap: '16px', marginTop: '12px', fontSize: '12px' }}>
                  {cloudVideoUrl && <a href={cloudVideoUrl} target="_blank" rel="noreferrer" style={{ color: 'var(--pink)' }}>Cloud copy</a>}
                  {localFinalUrl && <a href={localFinalUrl} target="_blank" rel="noreferrer" style={{ color: 'var(--text-3)' }}>Local copy</a>}
                </div>
              </div>
            </div>
          )}
        ...
        <aside style={{
          ...
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--text-3)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '12px' }}>
            Recent Reels
          </div>

          {historyLoading && <div style={{ fontSize: '12px', color: 'var(--text-3)' }}>Loading...</div>}
          {historyError && <div style={{ fontSize: '12px', color: 'var(--pink)' }}>{historyError}</div>}
